...
//  mellomtittel (lange tekster: historier, «les mer»), `data-format="inline"`
//  har bare fet og kursiv (felt der hver linje ER et punkt, v5.50), alt annet
//  får det korte settet. Feltene leses fortsatt med .value som før — formatlinja rører kun
...
const KNAPPER = [
  ...
  { id: "ul", tittel: "Punktliste (- punkt)", html: "• Liste", blokk: true, handling: (ta, cb) => prefixLines(ta, () => "- ", cb) },
  { id: "ol", tittel: "Nummerert liste (1. punkt)", html: "1. Liste", blokk: true, handling: (ta, cb) => prefixLines(ta, (i) => `${i + 1}. `, cb) },
  { id: "h3", tittel: "Mellomtittel (### Tittel)", html: "Mellomtittel", blokk: true, kunFull: true, handling: (ta, cb) => prefixLines(ta, () => "### ", cb) },
];

// Sett formatlinja over ett tekstfelt. Idempotent: kalles trygt på nytt når en
// modal åpnes igjen (feltet merkes med data-fmt).
function attachFormatBar(ta, { full = false, inline = false, onChange } = {}) {
...
export function setupFormatBars(root = document) {
  root.querySelectorAll("textarea[data-format]").forEach((ta) =>
    attachFormatBar(ta, { full: ta.dataset.format === "full", inline: ta.dataset.format === "inline" }));
}
