...
import {
  ...
  DECADES,
  INSTRUMENTS,
} from "./limits.js?v=5.37";
import { escapeHtml, GENDER_LABEL, pct, teacherActionRow, toggleCheckBtn, PRIO_ICONS, PRIO_LABELS } from "./ui-helpers.js?v=5.37";
import { GENEALOGY, GENEALOGY_MAIN_GENRES, GENEALOGY_META_GENRES, GENEALOGY_EDGES, edgeKey, isMainGenre } from "./genre-model.js?v=5.37";
import { resolveDesc, resolveDescAny } from "./genre-descriptions.js?v=5.37";
import { storyOrder, storyFor, pageFor } from "./story-format.js?v=5.37";
...
export function contentGaps({ artists = [], genreDescs = {}, edgeDescs = {}, content = {}, contentLoaded = false }) {
  ...
  // Tekstløse dokumenter teller som hull (v5.32, audit-funn 7).
  const pages = contentLoaded ? ["rotter", "omHistorie"].filter((id) => !pageFor(id, content)?.body?.trim()) : [];
...
export function renderDashboard(el, {
  ...
  // Artister uten grad står IKKE i kortet — de hører hjemme under «Innhold som
  // mangler» nederst, sammen med det andre som gjenstår å fylle ut.
  const prioLists = {
  ...
  const utenPrio = active.filter((a) => !a.priority).sort(byName);
  ...
  // --- Instrument -------------------------------------------------------------
  // Kun instrumenter som faktisk er i bruk: en boble med 0 forteller ingenting
  // om samlingen, den forteller bare hva vokabularet inneholder. Verdier utenfor
  // vokabularet blir stående så lenge noen bruker dem — de er datafeil, og
  // flagges dessuten i «Innhold som mangler».
  const instruments = [
    ...INSTRUMENTS,
    ...Object.keys(counts.perInstrument)
      .filter((i) => i && i !== "undefined" && !INSTRUMENTS.includes(i)),
  ].filter((i) => (counts.perInstrument[i] || 0) > 0);
  ...
  // Grønt lys krever TEKST, ikke bare et dokument (v5.32, audit-funn 7).
  const pageStatus = (id) => (contentLoaded ? !!pageFor(id, content)?.body?.trim() : null);
  ...
  el.innerHTML = `
    ...
    <div class="stat-card ov-block">
      ...
      <div class="ov-chips">${instruments.map((i) => {
        ...
        return `<button type="button" class="ov-chip" data-ov-instr="${escapeHtml(i)}">${escapeHtml(i)} <strong>${n}</strong></button>`;
      }).join("")}</div>
    </div>
