...
import { state, ctx, openAdminModal, closeAdminModal, lukkEtter, avbrytLukkEtter, setContentCheck, guardTeacherAction } from "./teacher-state.js?v=5.59";
import { saveDecadeDesc, saveGenreDescLevel, saveEdgeDesc, saveStoryBody, clearStory, savePage, deletePage, saveReferanser, addTech, updateTech, deleteTech, addPodcast, updatePodcast, deletePodcast } from "./store.js?v=5.59";
import { resolveMainDesc } from "./genealogy.js?v=5.59";
import { dropboxDirectUrl } from "./util.js?v=5.59";
import { GENEALOGY, edgeKey } from "./genre-model.js?v=5.59";
import { storyFor, pageFor } from "./story-format.js?v=5.59";
import { renderRichText } from "./rich-text.js?v=5.59";
import { wrapSelection, prefixLines } from "./format-bar.js?v=5.59";
import { escapeHtml, buildKilderList, buildMainGenreList, renderDecadeSections, renderDecadeRibbon, setupModal, modalOpen, techImage, fillSelect } from "./ui.js?v=5.59";
import { resolveDesc } from "./genre-descriptions.js?v=5.59";
import { renderPodcastList, wirePlayerCloseGuard, wireCharCount, checkBtnHtml, toggleCheckBtn, teacherActionRow, wireTeacherRow, techFactsLines, ICONS, fyllPunktfelt, lesPunktfelt } from "./ui-helpers.js?v=5.59";
import { DECADES, DECADE_OPTIONS, INSTRUMENT_TIMELINE_GROUPS, INSTRUMENT_TITLE, instrumentPageId, SAMMENDRAG_MAKS } from "./limits.js?v=5.59";
import { heatRow, getHeatData } from "./heat-strip.js?v=5.59";

const LEVEL_LABEL = { meta: "metasjanger", main: "sjanger", sub: "undersjanger" };
import { wireAllLinks } from "./linkify.js?v=5.59";
import { $ } from "./shared.js?v=5.59";
import { SOURCE_SPEC, addRow, buildRows, collectRows, normalizeSources } from "./row-editor.js?v=5.59";
...
const PAGE_TITLES = { omHistorie: "Om historie", rotter: "Røtter før 1910", appGuide: "Slik bruker du appen", skriveveiledning: "Slik skriver du beskrivelsen" };
...
function openContentEditor(target, title, existing) {
  ...
  // Statusen følger TEKSTEN (v5.32, audit-funn 7): et dokument med bare
  // kilder (godkjent kildeforslag) skal ikke omtales som lagret tekst.
  const harTekst = !!existing?.body?.trim();
  $("#se-status").textContent = harTekst
