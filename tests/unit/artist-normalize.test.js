...
import { normalizeArtist } from "../../js/artist-normalize.js?v=5.48";
...
test("buildArtistDoc: returnert-status og returfeltene bevares ved import", async () => {
  const { buildArtistDoc } = await import("../../js/artist-normalize.js?v=5.48");
  const inn = {
...
test("resubmitArtistFields: skriver KUN feltene skjemaet sendte", async () => {
  const { resubmitArtistFields } = await import("../../js/artist-normalize.js?v=5.48");
