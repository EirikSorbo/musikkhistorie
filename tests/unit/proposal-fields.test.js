...
import { PROPOSABLE_KEYS, proposableKeysFor } from "../../js/proposal-fields.js?v=5.69";
...
test("returflyten: lekkasjefilter, regler, stempling og eksport henger sammen", async () => {
  ...
  const { ARTIST_FIELDS } = await import("../../js/artist-schema.js?v=5.69");
...
test("SKJUL_I_HUBEN stemmer med kortene i «Det store bildet»", async () => {
  const fs = await import("node:fs");
  const les = (f) => fs.readFileSync(new URL(`../../${f}`, import.meta.url), "utf8");
  const { SKJUL_I_HUBEN, SKJUL_I_STUDENTVISNING } = await import("../../js/feature-flags.js?v=5.69");
...
test("RETUR_FELTER er én kilde: eksport, buildArtistDoc og ryddReturfelter", async () => {
  const { RETUR_FELTER, ARTIST_EXPORT_FIELDS } = await import("../../js/artist-schema.js?v=5.69");
