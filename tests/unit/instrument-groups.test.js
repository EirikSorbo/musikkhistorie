...
} from "../../js/limits.js?v=5.23";
import { instrumentInnovations, buildInstrumentTimeline } from "../../js/ui-timeline.js?v=5.23";
import { PROPOSABLE_KEYS } from "../../js/proposal-fields.js?v=5.23";
...
test("instrumentPageId gir lovlige, stabile Firestore-ID-er", async () => {
  const { instrumentPageId } = await import("../../js/limits.js?v=5.23");
...
test("kort uten type ER en innovasjon — de 66 gamle trengte ingen migrering", async () => {
  const { techType, isHendelse } = await import("../../js/ui-tech.js?v=5.23");
...
test("begge typer havner på instrumenttidslinjen, kun hendelser merkes", async () => {
  const { buildInstrumentTimeline } = await import("../../js/ui-timeline.js?v=5.23");
  const kort = [
    { id: "a", name: "Elektrisk gitar", adoptedYear: 1938, instrument: "Gitar", status: "active" },
...
test("hendelseskort vises ikke i Teknologi-seksjonen", async () => {
  const { renderTechList } = await import("../../js/ui-tech.js?v=5.23");
  const el = { innerHTML: "" };
  const kort = [
