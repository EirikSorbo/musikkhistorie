...
import { escapeHtml, safeUrl, throttle, wikimediaThumb, WIKI_THUMB_WIDTHS, dropboxDirectUrl, kanoniskJson } from "../../js/util.js?v=5.39";
...
test("genererReturKode: lengde, alfabet og normalisering", async () => {
  const { genererReturKode, normaliserReturKode, RETUR_KODE_ALFABET, RETUR_KODE_LENGDE }
    = await import("../../js/util.js?v=5.39");
  for (let i = 0; i < 50; i++) {
...
test("TREG_SENDING_MELDING: delt, og uten tankestrek", async () => {
  const { TREG_SENDING_MELDING } = await import("../../js/util.js?v=5.39");
  const fs = await import("node:fs");
...
test("merkHarSendtInn/harSendtInn: normalvei og kastende localStorage", async () => {
  const { merkHarSendtInn, harSendtInn } = await import("../../js/util.js?v=5.39");
  const lager = new Map();
