...
} from "../../js/genre-migrate.js?v=5.19";
...
test("heatOrphanKeys: rot-noder uten metasjanger er IKKE foreldreløse", async () => {
  const { heatOrphanKeys } = await import("../../js/genre-migrate.js?v=5.19");
  const noder = [
...
test("heatOrphanKeys: matcher både etikett og fullnavn, og tåler tomt", () => {
  return import("../../js/genre-migrate.js?v=5.19").then(({ heatOrphanKeys }) => {
...
test("artistForslagOmskriving: navnebytte treffer alle fire flatene", async () => {
  const { artistForslagOmskriving } = await import("../../js/genre-migrate.js?v=5.19");
...
test("edgeOrphanKeys: motreaksjon teller som kant, rester fanges", async () => {
  const { edgeOrphanKeys } = await import("../../js/genre-migrate.js?v=5.19");
  const noder = [
    { id: "a", l: "A", p: [], rx: [] },
    { id: "b", l: "B", p: ["a"], rx: [] },
    { id: "c", l: "C", p: [], rx: ["a"] },
